...
for (var key in data) {
	if (REGEXP.test(key)) {
		...
		for (month = 0; month < months; month++) {
			month_str = moment([2001, 0, 1]).add(month, 'month').format('YYYY-MM');
...
var spreadsheetRows = [];
var spreadsheetRows_monthlyVariation = [];


var ALL_FIELDS = SCM_FIELDS.concat(ITS_FIELDS).concat(MLS_FIELDS);

var HEADER = ['project', 'date'].concat(ALL_FIELDS);

spreadsheetRows.push(HEADER);
spreadsheetRows_monthlyVariation.push(HEADER);
